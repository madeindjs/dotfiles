-- various audio visualization from https://github.com/mfcc64/mpv-scripts/blob/master/visualizer.lua
...
local function visualizer_hook()
	...
	if atrack == nil and vtrack == nil then
		for id, track in ipairs(mp.get_property_native("track-list")) do
			if
				track.type == "video"
				and (vtrack == nil or vtrack.albumart == true)
				and mp.get_property("vid") ~= "no"
			then
				vtrack = track
